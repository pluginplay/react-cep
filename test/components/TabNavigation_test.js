...
it('renders the configured tabs', () => {
  ...
  expect(navigationButtons.at(0).find('img').first()).to.have.prop('src').equal('one')
  expect(navigationButtons.at(1).find('img').first()).to.have.prop('src').equal('two')
  expect(navigationButtons.at(2).find('img').first()).to.have.prop('src').equal('three')
